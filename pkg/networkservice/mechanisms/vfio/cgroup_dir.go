...
const (
	cgroupInfoFile = "/proc/1/cgroup"
)

var devicesCgroup = regexp.MustCompile("^[1-9][0-9]*?:devices:(.*)$")

// CgroupDirPath returns cgroup directory path on the host machine for the container
func CgroupDirPath() (string, error) {
	cgroupInfo, err := os.OpenFile(cgroupInfoFile, os.O_RDONLY, 0)
	if err != nil {
